...
\subsection{Результаты}
    ...
    Компиляция программы проводится с помощью gcc.
...
\subsection{Введение в алгоритм}
    ...
    (Избегать таким образом букву М в отображениях)
    ...
    Вопросы возникли после того, как я посмотрел на таблицу
    ...
    \begin{center}
        \begin{tikzpicture}[
            ...
            \begin{scope}[shorten >=7pt,shorten <= 7pt]
            \draw[->]  (m-6-6.center) -- (m-5-6.center) -- (m-4-5.center) --
                       (m-4-4.center) -- (m-4-3.center) -- (m-3-2.center) --
                       (m-2-2.center);
            \end{scope}
        \end{tikzpicture}
        $\Rightarrow$
        $M = \frac{1}{k}\sum_{i=1}^k(x_i-y_{j(i)})^2 = \frac{5}{7}((0.1)^2) \simeq 0.007$
    \end{center}

    \begin{enumerate}
        \item По-моему, одно число не очень ясно дает понять насколько
        хорошо сработал алгоритм. Может стоит также отдавать 
        свдиг, с которого вторая последовательность становится
        хорошо сопоставлена с первой. На примере с непрерывной
        функцией выше понятнее всего.

        \item Попробовать визуализировать отображения?
        Нарисовать графики последовательностей аналогично тем же
        непрерывным функциям с правильным сдвигом.
        \item Пытаюсь смоделировать разные примеры. Думаю, что наверное
        лучше при последовательностях разной длины не стараться
        отразить правильно все точки второй в точки первой,
        как это сделано в примере (b), то есть
        иметь $\leq$ длины второй последовательности отображений.

        Пример (b) \[ S1 = \{1,\ 3.4,\ 8,\ 3.6,\ 1\},\ S2 = \{3.5,\ 9,\ 2.8\} \]
        \begin{center}
            \begin{tikzpicture}[ele/.style={fill=black,minimum width=.8pt,inner sep=1pt}]
                \node[ele,label=above:$1$]   (a0) at (0,1) {};
                \node[ele,label=above:$3.4$] (a1) at (1,1) {};
      ...
      \begin{tikzpicture}[
        ...
        \matrix (m) [table] {
                & $1$   & $3.4$ & $8$ & $3.6$ & $1$ \\
          $3.5$ &  6.25 & 0.01 & 20.25 & 0.01  & 6.25  \\
          $9  $ &  64   & 31.36 &    1 & 29.16 &    64  \\
          $2.8$ &  3.24 & 0.36 & 27.04 & 0.64  & 3.24  \\
        };
    \end{tikzpicture}

    То есть остается понять как правильно выбрать элемент,
    с которого начнется отображение.

    Можно отдельно упорядочить последнюю строчку, 
    и идти с минимального, посчитать для всех $M$,
    тот, у кого это $M$ минимальное, тот и выиграл.
